...
const navLinks = [
  ...
  { label: "Layanan",    href: "/layanan",     isAnchor: false },
  { label: "Portofolio", href: "/portofolio",  isAnchor: false },
  { label: "Tentang",    href: "/tentang",     isAnchor: false },
      ...
      >
        <div className="max-w-6xl mx-auto px-6 flex items-center justify-between h-16">
          ...
          <div className="hidden md:flex items-center gap-1">
            {navLinks.map((link) => {
              ...
              const className = `px-3 py-1.5 text-sm font-medium rounded-lg transition-colors duration-200 no-underline ${
                isActive
        ...
        >
          <div className="px-6 py-4 flex flex-col gap-1 bg-white">
            {navLinks.map((link) => {
              const isActive = !link.isAnchor && pathname === link.href;
              const className = `px-3 py-2.5 text-sm font-medium rounded-lg transition-colors no-underline ${
                isActive
                  ? "text-[#C0272D] bg-[#FBF7F7] font-semibold"
                  : "text-[#3B3B58] hover:text-[#C0272D] hover:bg-[#FBF7F7]"
              }`;
