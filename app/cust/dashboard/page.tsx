    ...
    <div style={{ minHeight: "100vh", background: "#f8fafc", display: "flex", fontFamily: "var(--font-plus-jakarta-sans), 'Inter', sans-serif" }}>
      <aside style={{
        ...
        <div style={{
          ...
          <span style={{ fontWeight: 800, fontSize: "1.05rem", color: "#0f172a" }}>Customer Portal</span>
        </div>
        ...
        <nav style={{ padding: "8px 16px", flex: 1, display: "flex", flexDirection: "column", gap: 4 }}>
          {[
            ...
            { icon: "🛍️", label: "My Orders", active: false },
        ...
        <header className="dash-fadein" style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 36 }}>
          <div>
            <div style={{
              ...
              <span>{user.icon}</span>
              {user.majorLabel}
              <span style={{ background: "#f3f4f6", color: "#374151", padding: "2px 8px", borderRadius: 12, fontSize: "0.68rem" }}>External</span>
            </div>
            ...
            <p style={{ color: "#64748b", fontSize: "0.95rem", lineHeight: 1.6 }}>Track your project orders and communicate with the TEFA team.</p>
          </div>
        </header>

        <div className="dash-fadein" style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 20, marginBottom: 32 }}>
          {[
            { label: "Active Orders", value: "1", icon: "🛍️" },
            { label: "Total Spent", value: "$420", icon: "💳" },
